...
export default function NumberButtons(){
  ...
  useEffect(() => {    //inital rendering 
    if (numbers.length >= 2) {
      ...
      let index2 = Math.floor(Math.random() * (numbers.length - 1));
      if (index2 === index1) {
        index2++;
      }
      ...
      listAll(storageRef).then((res) => {
        ...
        res.items.forEach((itemRef, index=0) => {
          ...
          getDownloadURL(itemRef).then((url) => {
            ...
            setMonkey(prevImageData => ({
              ...prevImageData,
              [index]: { name: humanize(itemRef.name.substring(0,itemRef.name.length - 4)), url: url, tier: 1, idx: index }
            }));
  ...
  useEffect(() => {
    if(Object.keys(monkey).length === 64) {
      setDisplay(true);
    } else {
      setDisplay(false);
      console.log(Object.keys(monkey).length)
    }
  })
